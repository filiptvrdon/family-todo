...
1. **Check the spec first.** Look in `specs/`. If a matching spec exists with status `ready`, follow it. Update status to `in-progress` when starting, `done` when finished. Fill in Implementation notes as you go.
...
`specs/` contains one `.md` file per feature. Use `specs/_template.md` to create new ones.
